...
2. **Извлечение embeddings** — Python-скрипт с InsightFace (модель `buffalo_l`) детектирует лица и возвращает 512-мерные нормализованные embedding-векторы. В GPU-режиме запускается один процесс с потоковым вводом/выводом (batch), на CPU — параллельный worker pool
3. **Генерация миниатюр** — для каждого обнаруженного лица вырезается crop с паддингом 25%, масштабируется до 160×160 и сохраняется в `.thumbnails/`
4. **Кластеризация** — Go вычисляет косинусное сходство между всеми парами embeddings и группирует лица через Union-Find (disjoint set с path compression и union by rank)
...
8. **Веб-интерфейс (опционально)** — Go HTTP-сервер с тёмной темой для просмотра результатов в браузере
...
Встроенный HTTP-сервер с тёмной темой для просмотра результатов:
...
Два режима работы:
- **GPU (batch)** — один Python-процесс, пути передаются через stdin, результаты читаются из stdout (JSONL). Модель загружается однократно
...
Для каждой пары лиц вычисляет косинусное сходство embeddings. Если сходство >= порога — объединяет через Union-Find (disjoint set с path compression и union by rank). Сложность: O(n²) по числу лиц.
...
Структурированный JSON-отчёт с метриками обработки: количество изображений, лиц, персон, ошибок, время, пороги. Для каждой персоны — количество фото, миниатюра, описание, список путей к фото.
...
Встроенный HTTP-сервер (Go `net/http` + `embed`). Отдает HTML-страницу с JavaScript, который загружает `report.json` через API и рендерит интерфейс: карточки персон, сетку фотографий, полноэкранный просмотр.
